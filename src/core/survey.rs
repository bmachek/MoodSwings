...
mod tests {
    ...
    // >>> TEMP DIAG START (remove me)
    // ------------------------------------------------ TEMPORARY DIAGNOSTIC --

    fn in_polygon(points: &[Vec2], at: Vec2) -> bool {
        let mut inside = false;
    ...
    fn seg_distance(a: Vec2, b: Vec2, p: Vec2) -> f32 {
        let d = b - a;
    ...
    #[test]
    fn temp_gap_diagnosis() {
        let Some(town) = atlas::load("landshut") else {
            panic!("no atlas");
        };
        let seed = GameConfig::default().world_seed;
        let half_extent = GameConfig::default().world.half_extent;
        let (mut layout, signs) = atlas::layout(&town, seed, half_extent);
        let real = atlas::footprints(
            &town,
            &layout.graph,
            seed,
            half_extent,
        ...
        let (blocks, _holes) = streetside::lots(&layout, seed, CityStyle::Landshuepf, real);
        layout.blocks = blocks;
        let walls = Walls::build(&layout, &mapped_parts);

        // Every raw footprint the bake recorded, as a wall, with whether it
        // survived `footprints` (the hill / the square) and whether it survived
        // `lots` (the road).
        struct Raw {
            wall: Wall,
            kept_footprints: bool,
            kept_lots: bool,
            uphill: bool,
            name: String,
        }
        let kept_lots: std::collections::HashSet<PartKey> = layout
            .blocks
            .iter()
        ...
        for f in &town.buildings {
            let centre = Vec2::new(f.centre.0, f.centre.1);
            if centre.x.abs() > half_extent + 40.0 || centre.y.abs() > half_extent + 40.0 {
                continue;
            }
            let axis = Vec2::new(f.yaw.cos(), -f.yaw.sin());
            let wall = Wall {
                centre,
                axis,
                half: Vec2::new(f.frontage, f.depth) * 0.5,
                real: true,
            };
            // Find it in what survived: the key is the *turned* footprint, so
            // match by nearest centre instead.
            let mut kept_fp = false;
            let mut kept_l = false;
            for (key, _) in mapped_parts.iter() {
                let at = Vec2::new(key.0 as f32 / 10.0, key.1 as f32 / 10.0);
                if at.distance(centre) < 0.6 {
                    kept_fp = true;
                    kept_l = kept_lots.contains(key);
                    break;
                }
            }
            raws.push(Raw {
                wall,
                kept_footprints: kept_fp,
                kept_lots: kept_l,
                uphill: relief.is_some_and(|r| r.is_hill(centre)),
                name: f.name.clone(),
            });
        }
        let mut raw_grid: HashMap<(i32, i32), Vec<usize>> = HashMap::default();
        for (i, raw) in raws.iter().enumerate() {
            let reach = Vec2::splat(raw.wall.half.x + raw.wall.half.y);
            let (low, high) = (
                cell_of(raw.wall.centre - reach),
                cell_of(raw.wall.centre + reach),
            );
            for x in low.0..=high.0 {
                for z in low.1..=high.1 {
                    raw_grid.entry((x, z)).or_default().push(i);
                }
            }
        }
        ...
        enum Why {
            MapBuildingLostToHill,
            MapBuildingLostToRoad,
            MapBuildingOffSquare,
            Water,
            Ground(u8),
            Hill,
            Bare,
        }
        let ground_name = |k: u8| match k {
            0 => "Grass",
            1 => "Park",
            2 => "Cemetery",
            3 => "Trees",
        ...
        let ground_code = |k: atlas::GroundKind| -> u8 {
            use atlas::GroundKind::*;
            match k {
                Grass => 0,
                Park => 1,
                Cemetery => 2,
        ...
        let classify = |kerb: Vec2, outward: Vec2| -> Why {
            // The map's own building, wherever it ended up.
            let to = kerb + outward * WALL_REACH;
            let (low, high) = (cell_of(kerb.min(to)), cell_of(kerb.max(to)));
            let mut found: Option<&Raw> = None;
            'cells: for x in low.0..=high.0 {
                for z in low.1..=high.1 {
                    for &i in raw_grid.get(&(x, z)).into_iter().flatten() {
                        let raw = &raws[i];
                        if raw.wall.struck_by(kerb, outward, WALL_REACH) {
                            found = Some(raw);
                            break 'cells;
                        }
                    }
            ...
            if let Some(raw) = found {
                return if !raw.kept_footprints {
                    if raw.uphill {
                        Why::MapBuildingLostToHill
                    } else {
                        Why::MapBuildingOffSquare
                    }
                } else if !raw.kept_lots {
                    Why::MapBuildingLostToRoad
                } else {
                    // Standing, but the ray that found it in the raw set did
                    // not find it in the walls: it was turned by `footprints`.
                    Why::MapBuildingLostToRoad
                };
            }
            let probes = [
            ...
            for arm in &layout.waters {
                for pair in arm.points.windows(2) {
                    if probes
                        .iter()
                        .any(|p| seg_distance(pair[0], pair[1], *p) <= arm.width * 0.5)
                    {
                        return Why::Water;
                    }
                }
            ...
            for ground in &layout.grounds {
                for p in &probes {
                    ...
                    if p.x >= b.min.x
                        ...
                        && p.y >= b.min.y
                        && p.y <= b.max.y
                        && in_polygon(&ground.points, *p)
                    {
                        return Why::Ground(ground_code(ground.kind));
                    }
                }
            }
            if relief.is_some_and(|r| probes.iter().any(|p| r.is_hill(*p))) {
                return Why::Hill;
            }
            Why::Bare
        };

        // Walk exactly as `measure_closure` does.
        let graph = &layout.graph;
        let mut tally: std::collections::BTreeMap<Why, usize> = Default::default();
        let mut tally_core: std::collections::BTreeMap<Why, usize> = Default::default();
        let mut hits = 0usize;
        let mut samples = 0usize;
        // Bare samples, split by how far the nearest thing the map recorded is.
        let mut bare_by_distance: std::collections::BTreeMap<&str, usize> = Default::default();
        let mut bare_backland = 0usize;
        let nearest_raw = |kerb: Vec2| -> f32 {
            let mut best = f32::INFINITY;
            let rings = (0..=6).map(|r| r as i32);
            for r in rings {
                let c = cell_of(kerb);
                for x in (c.0 - r)..=(c.0 + r) {
                    for z in (c.1 - r)..=(c.1 + r) {
                        if r > 0 && (x - c.0).abs() != r && (z - c.1).abs() != r {
                            continue;
                        }
                        for &i in raw_grid.get(&(x, z)).into_iter().flatten() {
                            let raw = &raws[i];
                            let d = (raw.wall.centre.distance(kerb)
                                - raw.wall.half.x.max(raw.wall.half.y))
                                .max(0.0);
                            best = best.min(d);
                        }
                    }
                }
                if best.is_finite() && best < (r as f32) * CELL {
                    break;
                }
            }
            best
        };
        // Per-gap detail, keyed by the gap's middle.
        let mut gap_detail: Vec<(f32, EdgeId, Option<String>, Vec2, Vec<Why>, Vec<f32>)> =
            Vec::new();
        ...
        for (points, edges) in streetside::street_runs(&layout) {
            ...
            for side in [-1.0f32, 1.0] {
                let mut open: Option<(f32, Vec<Why>, Vec<f32>)> = None;
                let mut along = 0.0f32;
                while along <= total {
                    let (i, point) = locate(along);
                    let (edge, out, axis) = segments[i];
                    let clear = (head && along < JUNCTION_CLEAR)
                        ...
                        || out >= INNER;
                    if clear {
                        if let Some((from, whys, depths)) = open.take() {
                            let metres = whys.len() as f32 * KERB_STEP;
                            let (i, at) = locate(from + metres * 0.5);
                            gap_detail.push((metres, segments[i].0, None, at, whys, depths));
                        }
                        along += KERB_STEP;
                        continue;
                    }
                    ...
                    let outward = Vec2::new(-axis.y, axis.x) * side;
                    let kerb = point + outward * (width * 0.5);
                    let (any, _real) = walls.cast(kerb, outward, WALL_REACH);
                    samples += 1;
                    hits += usize::from(any);
                    if any {
                        if let Some((from, whys, depths)) = open.take() {
                        ...
                        let why = classify(kerb, outward);
                        if why == Why::Bare {
                            let d = nearest_raw(kerb);
                            let bucket = if d < 25.0 {
                                "map building 18-25 m out"
                            } else if d < 40.0 {
                                "map building 25-40 m out"
                            } else if d < 70.0 {
                                "map building 40-70 m out"
                            } else if d.is_finite() {
                                "map building 70-120 m out"
                            } else {
                                "no map building inside 120 m"
                            };
                            *bare_by_distance.entry(bucket).or_default() += 1;
                            // Is the far side of this street built? If the
                            // sample opposite finds a wall, this is the inside
                            // of a block rather than a street with no wall.
                            let back = point - outward * (width * 0.5);
                            if walls.cast(back, -outward, WALL_REACH).0 {
                                bare_backland += 1;
                            }
                        }
                        *tally.entry(why).or_default() += 1;
                        if out < CORE {
                            *tally_core.entry(why).or_default() += 1;
                        }
                        // How far to the nearest standing wall, straight out.
                        let mut depth = f32::INFINITY;
                        for extra in 1..12 {
                            let far = extra as f32 * 10.0;
                            if walls.cast(kerb, outward, far).0 {
                                depth = far;
                                break;
                            }
                        }
                        match &mut open {
                            ...
                            None => open = Some((along, vec![why], vec![depth])),
                        }
                    }
                    along += KERB_STEP;
                }
        ...
        println!(
            "samples {samples}  hits {hits}  share {:.3}",
            hits as f32 / samples as f32
        );
        let miss: usize = tally.values().sum();
        println!(
            "--- why the {miss} misses ({} m of kerb) ---",
            miss as f32 * KERB_STEP
        );
        let mut rows: Vec<(&Why, &usize)> = tally.iter().collect();
        rows.sort_by_key(|(_, n)| std::cmp::Reverse(**n));
        for (why, n) in rows {
            let core = tally_core.get(why).copied().unwrap_or(0);
            let label = match why {
                Why::Ground(k) => format!("Ground({})", ground_name(*k)),
                other => format!("{other:?}"),
            };
            println!(
                "  {label:28} {:6} samples  {:7.0} m  {:5.1}% of misses   (core {core})",
                n,
                *n as f32 * KERB_STEP,
                *n as f32 / miss as f32 * 100.0
            );
        }

        println!("--- the {} bare samples by what the map has behind them ---", bare_by_distance.values().sum::<usize>());
        let mut brows: Vec<(&&str, &usize)> = bare_by_distance.iter().collect();
        brows.sort_by_key(|(_, n)| std::cmp::Reverse(**n));
        for (bucket, n) in brows {
            println!("  {bucket:32} {n:6} samples  {:7.0} m", *n as f32 * KERB_STEP);
        }
        println!("  of those, {bare_backland} samples ({:.0} m) have a wall on the OTHER side of the same street", bare_backland as f32 * KERB_STEP);

        // The longest stretches that are bare rather than river, park or hill.
        let mut bare_gaps: Vec<(usize, &(f32, EdgeId, Option<String>, Vec2, Vec<Why>, Vec<f32>))> = gap_detail
            .iter()
            .map(|g| (g.4.iter().filter(|w| **w == Why::Bare).count(), g))
            .filter(|(n, _)| *n > 0)
            .collect();
        bare_gaps.sort_by_key(|(n, _)| std::cmp::Reverse(*n));
        println!("--- the twelve longest BARE stretches ---");
        for (n, g) in bare_gaps.iter().take(12) {
            let name = signs
                .per_edge
                .get(g.1.0 as usize)
                .copied()
                ...
                .unwrap_or_default();
            println!(
                "{:5.0} m bare  {name:32} edge {:5}  at ({:.0},{:.0})  (gap is {:.0} m all told)",
                *n as f32 * KERB_STEP,
                g.1.0,
                g.3.x,
                g.3.y,
        ...
        gap_detail.sort_by(|a, b| b.0.total_cmp(&a.0));
        println!("--- the twelve longest gaps ---");
        for (metres, edge, _, at, whys, depths) in gap_detail.iter().take(12) {
            let name = signs
                .per_edge
                .get(edge.0 as usize)
                .copied()
                ...
                .map(|i| signs.names[i].clone())
                .unwrap_or_default();
            let mut counts: std::collections::BTreeMap<Why, usize> = Default::default();
            for why in whys {
            ...
            let nearest = finite.iter().copied().fold(f32::INFINITY, f32::min);
            let e = graph.edge(*edge);
            println!(
                "{metres:5.0} m  {name:32} edge {:5}  at ({:.0},{:.0})  width {:.1}  [{}]  nearest wall out {:.0} m ({}/{} samples find one inside 110 m)",
                edge.0,
                at.x,
                at.y,
                e.width,
